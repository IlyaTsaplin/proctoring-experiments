...
def main():
    for model_type in MODEL_CREATORS:
        model = MODEL_CREATORS[model_type]().create_model(
            input_shape=(8, 1), num_classes=2
        )
        ...
        for i, row in df.iterrows():
            values_flat = np.array(
                [int(char) for char in row["values"] if char in ["0", "1"]]
            )

            for _ in range(len(values_flat) // 8):
